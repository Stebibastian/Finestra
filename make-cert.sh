#!/bin/bash
# Richtet lautloses Code-Signing ein: ein EIGENER Signier-Schluesselbund mit festem
# Passwort, dem codesign dauerhaft vertraut (per set-key-partition-list).
# → Keine GUI-Nachfrage, kein Passwort-Tippen, stabile Signatur ueber Rebuilds.
# Der Schluesselbund enthaelt nur ein selbstsigniertes Wegwerf-Zertifikat (signiert
# nichts Vertrauenswuerdiges) - das feste Passwort ist daher unkritisch.
set -euo pipefail

NAME="Finestra Local Signing"
SIGN_KC="$HOME/Library/Keychains/finestra-signing.keychain-db"
KC_PW="finestra-local"
OPENSSL="/usr/bin/openssl"

# Schluesselbund anlegen (falls noetig) und immer entsperren - codesign braucht ihn offen.
if [ ! -f "$SIGN_KC" ]; then
...
# sonst koennen Eintraege verschmelzen und andere Schluesselbuende (z. B. der von gh
...
echo "→ Lege lokales Signatur-Zertifikat an (ohne Rueckfragen) …"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT

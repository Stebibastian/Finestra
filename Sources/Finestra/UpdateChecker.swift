...
/// Prueft die neueste GitHub-Release und vergleicht sie mit der eigenen Version.
/// Leichtgewichtig (nur die GitHub-API), kein externes Update-Framework.
enum UpdateChecker {
    private static let releasesAPI =
        "https://api.github.com/repos/Stebibastian/Finestra/releases/latest"
    static let releasesPage =
        "https://github.com/Stebibastian/Finestra/releases/latest"
    /// Holt + installiert die neueste notarisierte Version und startet sie neu.
    static let installCommand =
        "sleep 1; curl -fsSL https://raw.githubusercontent.com/Stebibastian/Finestra/main/web-install.sh | bash"

    static var currentVersion: String {
        Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? "0"
    }

    /// Fragt die neueste Release ab. `completion` laeuft auf dem Main-Thread:
    /// .success(info) = Update verfuegbar, .success(nil) = bereits aktuell, .failure = Fehler.

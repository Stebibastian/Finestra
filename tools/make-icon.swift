...
// Erzeugt das Finestra-App-Icon (tuerkis-gruenes Squircle + weisses Fenster-Symbol)
// als .iconset-Ordner. Aufruf: swift make-icon.swift <ziel-iconset-ordner>
...
func tintedSymbol(pointSize: CGFloat, color: NSColor) -> NSImage? {
    ...
    guard let base = NSImage(systemSymbolName: "macwindow", accessibilityDescription: nil)?
...
func renderIcon(pixel: CGFloat) -> Data? {
    ...
    let gradient = NSGradient(colors: [
        NSColor(srgbRed: 0.20, green: 0.78, blue: 0.72, alpha: 1),
        NSColor(srgbRed: 0.10, green: 0.52, blue: 0.62, alpha: 1),
    ])
    gradient?.draw(in: path, angle: -90)

    if let symbol = tintedSymbol(pointSize: pixel * 0.50, color: .white) {
